...
const otfToTtf = () => {
  ...
  return (
    gulp
      .src("site/src/fonts/*.otf", { encoding: false })
      .pipe(
        plumber(
          notify.onError({
            title: "FONTS",
...
const ttfToWoff = async () => {
  // Поиск файлов .ttf
  return await gulp
    .src("site/dest/fonts/ttf/*.ttf", { encoding: false })
    .pipe(
      plumber(
        notify.onError({
          title: "FONTS",
...
const fontsStyle = async () => {
  const fontsFile = "site/src/scss/imports/fonts.scss";
  fs.readdir("site/src/fonts", async function (err, fontsFiles) {
    if (fontsFiles) {
      await deleteAsync([
        "site/src/scss/imports/fonts.scss",
        "site/dest/fonts/ttf",
      ]);
